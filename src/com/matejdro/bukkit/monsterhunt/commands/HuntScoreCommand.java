...
public class HuntScoreCommand extends BaseCommand {
	...
	public Boolean run(CommandSender sender, String[] args) {
		if (args.length > 0 && args[0].equals("rank")) {
			Integer rank = InputOutput.getHighScoreRank(((Player) sender)
					.getName());
			if (rank != null)
				Util.Message(
						"Your current high score rank is "
								+ String.valueOf(rank), sender);
			else
			...
			LinkedHashMap<String, Integer> tops = InputOutput
					.getTopScores(number);
			...
			for (String player : tops.keySet()) {
				...
				Util.Message(rank + ". &6" + player + "&f - &a" + score
						+ "&f points", sender);
			}
		} else if (args.length > 0) {
			Integer score = InputOutput.getHighScore(args[0]);
			if (score != null)
				Util.Message("High score of player &6" + args[0] + "&f is &6"
						+ String.valueOf(score) + "&f points.", sender);
			else
				Util.Message("Player &6" + args[0]
						+ "&f do not have high score yet.", sender);
		} else {
			Integer score = InputOutput.getHighScore(((Player) sender)
					.getName());
			if (score != null)
				Util.Message("Your high score is &6" + String.valueOf(score)
						+ "&f points.", sender);
			else
				Util.Message("You do not have your high score yet.", sender);
		}
